# C2SMART Lab, NYU
# NCHRP 03-137
# @file    DRAC_Calculation_Online.py
# @author  Fan Zuo
# @author  Di Yang
# @date    2020-10-18
    ...
    # Main processing
        ...
        for p in range(len(df2)):
            for q in range(p+1, len(df2)):
                ...
                Loc0D1 = ttc_location_online(df_check0D1, Dist0D1, Start_time)
                Loc0D2 = ttc_location_online(df_check0D2, Dist0D2, Start_time)
                ...
                if dist(Loc0D1[0],Loc0D1[1],Loc0D2[0],Loc0D2[1]) < 50:
                    ...
                    Loc1D = ttc_location_online(df_check1D, Dist1D, Start_time)
                    Loc2D = ttc_location_online(df_check2D, Dist2D, Start_time)
                    Loc1D1 = Loc1D
                    Loc2D1 = Loc2D
                    ...
                    if overlap(Shape1D, Shape2D):
                        while DRAC_TTC > 0.0:
                            ...
                            # Obtain the TTC location
                            Loc1D = ttc_location_online(df_check1D, Back_dist1D, Start_time)
                            Loc2D = ttc_location_online(df_check2D, Back_dist2D, Start_time)
...
if __name__ == "__main__":
    ...
    if len(clean_results) != 0:
        ...
        Conflict.to_csv("DRAC_Online_"+"Time_"+str(Start_Point)+str(End_Point)+traj_file[:-4]+".csv")
    else:
