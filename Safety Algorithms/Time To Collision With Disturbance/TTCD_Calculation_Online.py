    ...
    # Main processing
        ...
        for p in range(len(df1)):
            for q in range(p+1, len(df1)):
                # Generate two random acceleration pools for Monte Carlo simulation, box_a for p-follow
                # mu, sigma = 0, 2
                runs = MC_Num
                ...
                # Find out when will the overlapped pair separate.
                    ...
                    for box_order in range (runs):
                        ...
                        Loc_check1 = ttc_location_online(df_check1, dist1v, Start_time)
                        Loc_check2 = ttc_location_online(df_check2, dist2v, Start_time)

                        if dist(Loc_check1[0],Loc_check1[1],Loc_check2[0],Loc_check2[1])<50:
                            # Obtain the TTC location
                            Loc1 = next_location_online(df_check1, 1, df_check1.at[0, 'X'], df_check1.at[0, 'Y'], float("{:.6f}".format(df_check1.at[0, 'Speed'] * 1.4667)), acc1, df1.at[p, 'Cal_heading'])
                            Loc2 = next_location_online(df_check2, 1, df_check2.at[0, 'X'], df_check2.at[0, 'Y'], float("{:.6f}".format(df_check2.at[0, 'Speed'] * 1.4667)), acc2, df1.at[q, 'Cal_heading'])
                            for time_step in range(1, int(TTCD_thr*10)+1):
                                ...
                                if overlap(Shape1, Shape2):
                                    ...
                                    if int(point_1) == len(df_check1):
                                        ...
                                        Loc1 = next_location_online(df_check1, point_1, X_new_1, Y_new_1, Speed_1, acc1, heading_1)
                                    if int(point_2) == len(df_check2):
                                        Loc2 = Loc2
                                    else:
                                        Loc2 = next_location_online(df_check2, point_2, X_new_2, Y_new_2, Speed_2, acc2, heading_2)
                        else:
                            pass
...
if __name__ == "__main__":
    ...
    if len(clean_results) != 0:
        ...
        Conflict.to_csv("TTCD_Online_"+str(ttcd_threshold)+"_"+str(mc_number)+"MC_"+"Time_"+str(Start_Point)+str(End_Point)+traj_file[:-4]+".csv")
    else:
